...
impl LifecycleRole {
    ...
    fn validate() -> Result<(), SidecarError> {
        ...
        if fs::metadata(state_root).is_ok_and(|metadata| metadata.is_dir())
            && fs::metadata(profile).is_ok_and(|metadata| metadata.is_file())
        {
            Ok(())
        } else {
